...
def test_app_config_ready_skips_management_commands():
    ...
    with patch("django.apps.config.AppConfig._path_from_module", return_value="/fake/path"):
        ...
        with patch("sys.argv", ["manage.py", "makemigrations"]):
            with patch.object(app_config, "initialize_mcp_server") as mock_init:
                app_config.ready()
...
def test_app_config_ready_initializes_for_runserver():
    ...
    with patch("django.apps.config.AppConfig._path_from_module", return_value="/fake/path"):
        ...
        with patch.object(app_config, "_should_skip_initialization", return_value=False):
            with patch.object(app_config, "_should_auto_discover", return_value=True):
                with patch("sys.argv", ["manage.py", "runserver"]):
                    with patch.object(app_config, "initialize_mcp_server") as mock_init:
                        with patch.object(app_config, "auto_discover_mcp_components") as mock_discover:
