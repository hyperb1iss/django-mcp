...
def test_register_drf_viewset_skips_if_drf_not_installed():
    ...
    with patch("django_mcp.drf_tools.DRF_AVAILABLE", False):
        with patch("django_mcp.drf_tools.logging.debug") as mock_debug:
...
def test_register_drf_viewset(mock_mcp_server, mock_drf_viewset):
    ...
    with patch("django_mcp.drf_tools.issubclass", return_value=True):
        # Fix importing ViewSet
        with patch("django_mcp.drf_tools.ViewSet", object):
            # Force DRF to be available
            with patch("django_mcp.drf_tools.DRF_AVAILABLE", True):
...
def test_register_drf_viewset_skips_non_viewsets(mock_mcp_server):
    ...
    # Force DRF to be available
    with patch("django_mcp.drf_tools.DRF_AVAILABLE", True):
        with patch("django_mcp.drf_tools.issubclass", return_value=False):
            register_drf_viewset(MagicMock())
...
def test_drf_action_tool():
    ...
    MockViewSetClass = MagicMock()
    MockViewSetClass.return_value.list = mock_list_action
    ...
    def drf_action_tool(**params):
        ...
        viewset_class = params.pop("_viewset_class", MockViewSetClass)
    ...
    result = drf_action_tool(_method="get", _action="list", _viewset_class=MockViewSetClass)
